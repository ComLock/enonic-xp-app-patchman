...
export function get(request) {
	...
	if (multirepoConnection) {
		...
		result.hits = result.hits.map(({
			...
			return {
				nodeId, score, repoId, branch: branchParam, node
			};
		});
	}
	//log.info(`modifyInReposWithIds:${toStr(modifyInReposWithIds)}`);
	...
	if (selectedFields.length) {
		result.hits = result.hits.map(({
			...
			return {
				nodeId, score, repoId, branch: branchParam, node: filteredNode
			};
		});
	}
	...
	if (actionParam === 'modify') {
		...
		Object.keys(modifyInReposWithIds).forEach((repoId) => {
			...
			modifyInReposWithIds[repoId].forEach((nodeId) => {
				...
				//allDiffsObj[repoId][branchParam][nodeId] = singlePatch;
				set(allDiffsObj, `${repoId}.${branchParam}.${nodeId}`, singlePatch);
			}); // each node in repo
		}); // each repo
	} // if modify
	...
	<body>
		...
		<form id="theForm" enctype="application/x-www-form-urlencoded" method="POST" style="margin-bottom: 0;">
			<details open>
				...
				<table style="width: 100%">
					<tbody>
						...
						<tr>
							<th><label for="repoIds">Branch</label></th>
							<td>
								<select name="branch" size="2">
									<option${branchParam === 'draft' ? ' selected' : ''} value="draft">draft</option>
									<option${branchParam === 'master' ? ' selected' : ''} value="master">master</option>
								</select>
							</td>
						</tr>
						<tr>
					...
					<input onclick="document.getElementById('actionQuery').checked = true;this.form.submit();" type="button" value="Query"/>
				</details>
				<details>
				...
				<input onclick="document.getElementById('actionModify').checked = true;this.form.submit();" type="button" value="Patch!"/>
			</details>
		</form>
